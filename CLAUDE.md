...
- **ESCF page (`public/escf.html`) implements the ESCF paper directly, not the notebooks.** The model lives in `public/escf_model.js` (shared with `scripts/tune_escf.js` for headless checks — run `node scripts/tune_escf.js 50` after touching it). Key tuning facts: duplex absent units must be faint (`absentAct=0.06`) or the shared absent baseline inflates every cosine and levels merge; each item needs **3 detail features** or its high-engagement trials snap into the basic hub (cos ≈ 0.91 > ρ=0.90 with only 2); the goal lens is squared (`goalSharpness=2`) and applied to present units only, otherwise an item's own perceptual hub outcompetes the affordance hub and empty-affordance items (daisy) spuriously match via shared absent units. Default seed 42 yields the clean 14-category hierarchy; the basic level is the softest across seeds, mirroring the paper.
